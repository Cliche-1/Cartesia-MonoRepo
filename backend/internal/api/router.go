...
func RegisterRoutes(app *fiber.App, db *gorm.DB, jwtSecret string) {
	...
	// Resources (upload & metadata)
	RegisterResourceRoutes(api, db, jwtSecret)

	// Search (advanced search & suggestions)
	RegisterSearchRoutes(api, db, jwtSecret)

	// Teacher application
	RegisterTeacherRoutes(api, db, jwtSecret)
}
